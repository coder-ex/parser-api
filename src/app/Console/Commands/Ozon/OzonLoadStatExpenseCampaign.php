...
class OzonLoadStatExpenseCampaign extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'ozon:stat-expense-campaign
    ...
    public function handle()
    {
        ...
        $dateTo = formatDateTo(date('Y-m-d H:i:s'), 'Y-m-d\TH:i:s.v\Z', 'UTC'); //date('Y-m-d');

        $task = $this->systemService->getTaskOne($typeDB, $project_id, TypeTask::StatExpenseCampaign->value);
        if (is_null($task)) {
        ...
        //$this->endpoint->requestStatMediaCampaign($table, $typeDB, $task->url, $project_id, $task->secret, $task->task, $task->dateFrom, $dateTo);
