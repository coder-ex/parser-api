...
class WbLoadSales extends Command
{
    /**
    ...
    public function handle()
    {
        ...
        $task = $this->systemService->getTaskOne($typeDB, $project_id, TypeTask::Sales->value);
        if (is_null($task)) {
            $this->error("Нет данных в настройках планировщика для [ Sales ]");
            $this->newLine();
            return 1;
        }
        ...
        // (new SalesService())->run($table, $typeDB, $task->url, $task->project_id, $task->secret, $task->task, $task->dateFrom, json_decode($task['extended_fields'])->flag);
