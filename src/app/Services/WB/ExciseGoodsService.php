...
use App\Services\WB\Processings\ExiseGoodProcessing;
use App\Services\WB\Traits\FetchDataFromAPI;
use ErrorException;
use Exception;

class ExciseGoodsService extends BaseService implements InterfaceService
{
     ...
     * @return void
     */
    public function run(string $table, string $typeDB, string $urlAPI, string $project, string $secret, string $task, ?string $from = null)
    {
        $sz = new Serializer($task, $this->debug);
        $processing = new ExiseGoodProcessing($table, $typeDB, $task);
    ...
    public function createUrl(string $urlAPI, ?string $secret = null, ?string $task = null, ?string $from = null): string
    {
        return $urlAPI . '/' . $task . '?key=' . $secret . '&dateFrom=' . $from;
    }
}
