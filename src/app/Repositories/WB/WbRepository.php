...
class WbRepository extends Repository
{
     ...
     * @return void
     */
    public function upsertIncomes(string $table, string $typeDB, array $data)
    {
        DB::connection($typeDB)->beginTransaction();
        try {
            DB::connection($typeDB)->table($table)->upsert(
                $data,
                ['incomeId', 'supplierArticle', 'techSize'],
     ...
     * @return void
     */
    public function upsertOrders(string $table, string $typeDB, array $data)
    {
        DB::connection($typeDB)->beginTransaction();
        try {
            DB::connection($typeDB)->table($table)->upsert(
